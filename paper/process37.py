...
def simple_condense_helper(bi, index):
    numtens = 0
    while index+1 < len(bi) and (bi[index:index+2]=='10' or bi[index:index+2]=='1o'):
        numtens += 1
        index += 2
    return "(10)^{"+str(numtens)+"}", index

def simple_condense(bi):
    index = 0
    retval = ""
    while index+1 < len(bi) and bi[index:index+2]=='10':
        st, index = simple_condense_helper(bi, index)
        retval += st
        if index < len(bi):
            retval += bi[index]
            index += 1
...
condensed_vers = []
for bitstring in bitstrings:
    condensed = simple_condense(bitstring) #condense(bitstring)
