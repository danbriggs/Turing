...
#print(len(bitstrings), "bitstrings at beginning:")
#for b in bitstrings:
#    print(b)
#print("")
...
def condense(bi):
    ...
    st = ""
    while index < len(bi):
        numTH, index = numrepshere(bi,'1110110', index)
        if numTH > 0:
            st += "(1110110)^{"+str(numTH)+"}"
            continue
        if index + 10 < len(bi) and bi[index:index+11]=="11111111111":
            st += "1^{11}"
            index += 11
            continue
        if index + 6 < len(bi) and bi[index:index+7]=="1111111":
            st += "1^{7}"
            index += 7
            continue
        if index + 5 < len(bi) and bi[index:index+6]=="111111":
            st += "1^{6}"
            index += 6
            continue
        st += bi[index]
